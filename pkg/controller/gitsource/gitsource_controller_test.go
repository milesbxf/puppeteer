...
func TestReconcile(t *testing.T) {
	...
	// Create the GitSource object and expect the Reconcile and CronJob to be created
	...
	g.Eventually(func() error { return c.Get(context.TODO(), cronjobKey, cronjob) }, timeout).
		Should(gomega.Succeed())

	// Delete the Deployment and expect Reconcile to be called for Deployment deletion
	g.Expect(c.Delete(context.TODO(), cronjob)).NotTo(gomega.HaveOccurred())
	g.Eventually(requests, timeout).Should(gomega.Receive(gomega.Equal(expectedRequest)))
	g.Eventually(func() error { return c.Get(context.TODO(), cronjobKey, cronjob) }, timeout).
		Should(gomega.Succeed())
